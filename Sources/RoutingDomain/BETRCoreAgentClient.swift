...
public actor BETRCoreAgentClient {
    ...
    public func clearSlot(outputID: String, slotID: String) async throws {
        ...
        guard assignedSourceIDIfPresent(
            outputID: outputID,
            slotID: slotID,
    ...
    public func applyLiveTileEvent(
        ...
        let preservedProgramReady =
            card.confidencePreview?.mode == .pendingProgram
            && card.confidencePreview?.sourceID == card.programSourceID
            ? (card.confidencePreview?.isReady ?? card.pendingProgramReady)
            : card.pendingProgramReady
        let preservedPreviewReady =
            card.confidencePreview?.mode == .armedPreview
            && card.confidencePreview?.sourceID == card.previewSourceID
            ? (card.confidencePreview?.isReady ?? false)
            : false
        ...
        card.confidencePreview = Self.makeConfidencePreview(
            ...
            programSourceReady: preservedProgramReady,
            previewSourceID: card.previewSourceID,
            previewSourceName: card.previewSourceName,
            previewSourceIsWarm: preservedPreviewReady,
    ...
    private func makeShellState(
        ...
        let cards = workspace.outputs.map { output in
            ...
            let previewSourceID = output.armedPreviewTile?.sourceID ?? output.previewSlotID.flatMap { slotID(for: $0, in: output) }
            let previewSourceName = output.armedPreviewTile?.sourceName ?? previewSourceID.flatMap { sourceNameByID[$0] }
            ...
            return RoomControlOutputCardState(
                ...
                confidencePreview: Self.makeConfidencePreview(
                    ...
                    programSourceReady: output.pendingProgramReady,
                    previewSourceID: previewSourceID,
                    previewSourceName: previewSourceName,
                    previewSourceIsWarm: output.armedPreviewTile?.ready ?? false
                )
            )
        }
    ...
    private func makeShellState(
        ...
        let outputTelemetryByID = Dictionary(uniqueKeysWithValues: (validation?.outputTelemetry ?? []).map { ($0.id, $0) })
        let proofOutput = validation?.proofOutput
        let proofOutputID = proofOutput?.outputID
        ...
        let cards = outputIDs.map { outputID in
            ...
            return RoomControlOutputCardState(
                ...
                isAudioMuted: telemetry?.audioPresenceState == .muted,
                isSoloedLocally: telemetry?.isSoloedLocally ?? false,
                pendingProgramReady: pendingProgramReady,
                selectedSourceFormatLabel: selectedSourceFormatLabel,
                statusPills: Self.makeStatusPills(
                    livePreviewState: liveTile.previewState,
                ...
                confidencePreview: Self.makeConfidencePreview(
                    ...
                    programSourceID: programSourceID,
                    programSourceName: programSourceID.flatMap { sourceNameByID[$0] },
                    programSourceReady: pendingProgramReady,
                    previewSourceID: previewSourceID,
                    previewSourceName: previewSourceID.flatMap { sourceNameByID[$0] },
                    previewSourceIsWarm: previewSourceReady
                )
            )
        }

        let unsortedSources: [RouterWorkspaceSourceState] = sourceRecords.map { record in
            let routedOutputIDs = cards.compactMap { card in
                if card.programSourceID == record.descriptor.id
                    || card.previewSourceID == record.descriptor.id
                    || card.liveTile.sourceID == record.descriptor.id {
                    return card.id
                }
                return nil
            }
            return RouterWorkspaceSourceState(
                id: record.descriptor.id,
                name: record.descriptor.name,
                details: sourceDetailsByID[record.descriptor.id] ?? "",
                provenance: record.provenance.rawValue,
                routedOutputIDs: routedOutputIDs,
                sortPriority: Self.makeSortPriority(
                    sourceID: record.descriptor.id,
                    programSourceID: cards.first(where: { $0.programSourceID == record.descriptor.id })?.programSourceID,
                    previewSourceID: cards.first(where: { $0.previewSourceID == record.descriptor.id })?.previewSourceID
                ),
        ...
        let sources = unsortedSources.sorted { lhs, rhs in
    ...
    private static func defaultOutputTitle(for outputID: String) -> String {
        let ordinal = outputID
            .split(separator: "-")
            .last
            .flatMap { Int($0) }
            ?? 1
        return ordinal == 1 ? "Program Output" : "Program Output \(ordinal)"
    }
