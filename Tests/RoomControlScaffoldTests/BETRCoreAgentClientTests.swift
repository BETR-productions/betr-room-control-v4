...
final class BETRCoreAgentClientTests: XCTestCase {
    ...
    func testBootstrapShellStateDoesNotTreatWarmSourceAsPendingProgramReady() async {
        let workspace = Self.makeWorkspaceSnapshot(
            outputs: [
                Self.makeWorkspaceOutput(
                    id: "OUT-1",
    ...
    func testBootstrapShellStateUsesArmedPreviewTileReadinessInsteadOfSourceWarmth() async {
        let workspace = Self.makeWorkspaceSnapshot(
            outputs: [
                Self.makeWorkspaceOutput(
                    id: "OUT-1",
    ...
    func testBootstrapValidationFallbackBuildsCardsFromRealOutputTelemetry() async {
    ...
    func testAssignSourceOnlySendsSlotAssignmentCommand() async throws {
        let transport = RecordingTransport()
    ...
    func testAssignSourceReplacesPreviewAssignmentWithoutSharedSourceLifecycleCommands() async throws {
        let transport = RecordingTransport()
        let snapshots = WorkspaceSnapshotSequence([
            Self.makeWorkspaceSnapshot(
                outputs: [
    ...
    func testAssignSourceAllowsIndependentOutputAssignmentWhenAnotherOutputAlreadyHasProgram() async throws {
        let transport = RecordingTransport()
        let snapshots = WorkspaceSnapshotSequence([
            Self.makeWorkspaceSnapshot(
                outputs: [
    ...
    func testAssignSourceRebindingLiveSlotOnlyUpdatesTheSlotAssignment() async throws {
        let transport = RecordingTransport()
        let snapshots = WorkspaceSnapshotSequence([
            Self.makeWorkspaceSnapshot(
                outputs: [
    ...
    private static func makeValidationSnapshot(
        ...
        outputSlots: [BETRCoreOutputSlotSnapshot]? = nil
        ...
        return BETRCoreValidationSnapshotResponse(
            ...
            outputSlots: outputSlots ?? (1...6).map { index in
                let slotID = "S\(index)"
                let sourceID: String?
                switch slotID {
                case "S1":
