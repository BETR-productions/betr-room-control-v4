...
- `0.3.31.1` is now the current staged date-line build for publish:
  - planned release: `https://github.com/BETR-productions/betr-room-control-v2/releases/tag/v0.3.31.1`
  - assets:
    - `BETR-Room-Control-v0.3.31.1.zip`
    - `BETR-Room-Control-v0.3.31.1.dmg`
  ...
  - the staged date-line build keeps the cold-start discovery bootstrap hotfix, the updater feed pagination fix, the live-program thumbnail/output-fallback recovery work, and the new house-timed program pipeline
  ...
  - publication uses explicit updater sequence `2026033101` so in-app update can advance cleanly from the bridge rescue line
...
- Important release truth:
  ...
  - the current staged date-line release is `.3.31.1`, canonicalized to `0.3.31.1`
  - explicit updater sequences `2026033009`, `2026033010`, `2026033011`, `2026033012`, and `2026033101` are what let the bridge rescue hand off cleanly across the date-line releases
